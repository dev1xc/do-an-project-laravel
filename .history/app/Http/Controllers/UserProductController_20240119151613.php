...
class UserProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if(empty($value['name']) && $value['category']=='0' && $value['brand']=='0' && $value['price']=='0-0') {
            $data = Product::paginate(9);
        }
        if(empty($value['name'])) {
            $data = Product::where('id_category', '=', $value['category'])->where('id_brand', '=', $value['brand'])->whereBetween('price', [$price[0], $price[1]])->paginate(9);
        }
        if ($value['category']=='0') {
            $data = Product::where('name', 'like', '%' . $value['name'] . '%')->where('id_brand', '=', $value['brand'])->whereBetween('price', [$price[0], $price[1]])->paginate(9);
        }
        if($value['brand']==0){
            $data = Product::where('name', 'like', '%' . $value['name'] . '%')->where('id_category', '=', $value['category'])->whereBetween('price', [$price[0], $price[1]])->paginate(9);
        }
        if($value['price']=='0-0'){
            $data = Product::where('name', 'like', '%' . $value['name'] . '%')->where('id_category', '=', $value['category'])->where('id_brand', '=', $value['brand'])->paginate(9);
        }
