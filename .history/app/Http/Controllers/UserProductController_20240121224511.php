...
class UserProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($value['category'])) {
            $query->where('id_category', $value['category']);
        }

        if (!empty($value['brand'])) {
            $query->where('id_brand', $value['brand']);
        }

        if (!empty($value['price'])) {
            $priceRange = explode('-', $value['price']);
    ...
    public function getPriceRange(Request $request)
    {
        $min = $request->get('min');
        $max = $request->get('max');
        $data = DB::table('product')->whereBetween('price', [$min, $max])->get();
        return redirect('/get-price-range?min='.$min.'&max='.$max)->with('success','success');
    }
