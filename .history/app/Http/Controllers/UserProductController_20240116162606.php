...
class UserProductController extends Controller
{
    ...
    public function slCart(Request $request) {
        ...
        session() -> put('data_product', $data);
        if(isset($_SESSION['array'][$idProduct])) {
            if(array_key_exists($idProduct, $_SESSION['array']	)) {
               $getData['qty'] = $_SESSION['array'][$idProduct]['qty'] + 1;
               $_SESSION['array'][$idProduct] = $getData;
            }
         }else {
            $_SESSION['array'][$idProduct] = $getData;
         }
