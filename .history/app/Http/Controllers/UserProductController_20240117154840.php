    ...
    public function create(Request $request)
    {
        ...
        if ($files = $request->file('image')) {
            foreach ($files as $file) {
                ...
                $path = public_path('upload/product/base'.$userId . $name);
                $path2 = public_path('upload/product/50'.$userId . $name_2);
                $path3 = public_path('upload/product/200'.$userId . $name_3);

                ImageManager::make($file->getRealPath())->save($path);
