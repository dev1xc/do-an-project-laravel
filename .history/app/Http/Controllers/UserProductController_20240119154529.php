...
class UserProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $data = Product::query();
        if($request->has('name')&& $value['name'] != '') {
            $data->where('name', 'like', '%' . $value['name'] . '%');
        }
        if($request->has('category')&& $value['category'] != '0'){
            $data->where('id_category', '=', $value['category']);
        }
        if($request->has('brand') && $value['brand'] != '0'){
            $data->where('id_brand', '=', $value['brand']);
        }
        if($request->has('price') && $value['price'] != '0') {
            $data->whereBetween('price', [$price[0], $price[1]]);
        }
        if($value['name']=='test' && $value['category'] == '0' && $value['brand'] == '0' && $value['price'] == '0') {
